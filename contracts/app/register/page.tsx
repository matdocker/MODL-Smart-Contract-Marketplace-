...
import { useTemplates } from '@/hooks/useTemplates'; // assuming you have this
import { useState } from 'react';
import ProjectModuleSkeleton from '@/components/ProjectModuleSkeleton';

export default function RegisterTemplatePage() {
  const { isConnected } = useAccount();
  const { submitTemplate, submittingTemplate, refetchTemplateIds } = useAuditRegistry();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('✅ Template submitted successfully and list refreshed!');
      setName('');
      setCategory('');
  ...
  if (!isConnected) {
    return (
      <div className="text-center mt-10 text-gray-500">
        Please connect your wallet to submit a template.
      </div>
    );
  }
