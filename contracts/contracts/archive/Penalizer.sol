...
import "@opengsn/contracts/src/interfaces/IPenalizer.sol";
...
contract Penalizer is IPenalizer, ERC165 {
    ...
    string public override versionPenalizer =
        "3.0.0-beta.3+opengsn.penalizer.ipenalizer";
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IPenalizer).interfaceId ||
    ...
    /// @inheritdoc IPenalizer
    function getPenalizeBlockExpiration()
        external
        view
        override
        returns (uint256)
    ...
    function isTransactionType1(
        bytes calldata rawTransaction
    ) internal pure returns (bool) {
        return (uint8(rawTransaction[0]) == 1);
    }

    function isTransactionType2(
        bytes calldata rawTransaction
    ) internal pure returns (bool) {
    ...
    function decodeTransaction(
        bytes calldata rawTransaction
    ) public pure returns (Transaction memory transaction) {
        if (isTransactionType1(rawTransaction)) {
            (
                ...
                transaction.value,
                transaction.data
            ) = RLPReader.decodeTransactionType1(rawTransaction);
            ...
            (
                ...
                transaction.value,
                transaction.data
            ) = RLPReader.decodeTransactionType2(rawTransaction);
        } else {
            (
                transaction.nonce,
                transaction.gasLimit,
                ...
                transaction.value,
                transaction.data
            ) = RLPReader.decodeLegacyTransaction(rawTransaction);
        }
        return transaction;
    }
    ...
    modifier commitRevealOnly() {
        bytes32 commitHash = keccak256(
            abi.encodePacked(keccak256(msg.data), msg.sender)
        );
        ...
        if (msg.sender != address(type(uint160).max)) {
            ...
            require(
                readyBlockNumber < block.number,
                "reveal penalize too soon"
            );
            require(
                readyBlockNumber + penalizeBlockExpiration > block.number,
    ...
    function _penalizeIllegalTransaction(
        ...
        if (isTransactionTypeValid(unsignedTx)) {
            ...
            if (decodedTx.to == address(hub)) {
                ...
                bool isWrongMethodCall = selector !=
                    IRelayHub.relayCall.selector;
